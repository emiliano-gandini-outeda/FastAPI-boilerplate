...
!!! note "The project ships without an initial revision"
    `backend/migrations/versions/` is intentionally empty — by default the schema is created on startup via `CREATE_TABLES_ON_STARTUP=true`. To switch to managed migrations, create the initial revision yourself against a fresh database:
